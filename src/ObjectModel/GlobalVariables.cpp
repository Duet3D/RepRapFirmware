...
void GlobalVariables::ReportAsJson(OutputBuffer *buf, ObjectExplorationContext& context, const ObjectModelClassDescriptor *_ecv_null classDescriptor, uint8_t tableNumber, const char *_ecv_array filter) const THROWS(GCodeException)
{
	if (*filter == 0)
	{
	...
	else
	{
		...
		const char *_ecv_array pos = GetNextElement(filter);				// find the end of the variable name
		const Variable *_ecv_null const var = vars.Lookup(filter, pos - filter, false);
		if (var == nullptr)
		{
			buf->cat("null");
		}
		...
		{
			ReportItemAsJson(buf, context, nullptr, not_null(var)->GetValue(), pos);
		}
		else
		{
			buf->cat("{}");
