...
{
	if(cantL > 0.0f && mode == 1) {
		return true;
	}
	else if(cantR > 0.0f && mode == 2) {
		return true;
	}
	return false;
}
...
//bool FiveBarScaraKinematics::LimitPosition(float position[], size_t numAxes, AxesBitmap axesHomed, bool isCoordinated) const
