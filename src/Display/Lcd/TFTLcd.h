...
public:
#if USE_FONT_CHIP
	TFTLcd(PixelNumber nr, PixelNumber nc, Pin fontCsPin, SpiMode mode, uint8_t sercomNum, uint32_t dataInPad, uint32_t dataOutPad) noexcept;
#else
	TFTLcd(PixelNumber nr, PixelNumber nc, const LcdFont * const fnts[], size_t nFonts, SpiMode mode, uint8_t sercomNum, uint32_t dataInPad, uint32_t dataOutPad) noexcept;
#endif
