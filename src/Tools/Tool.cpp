...
void Tool::IterateExtruders(function_ref_noexcept<void(unsigned int) noexcept> f) const noexcept
{
	for (size_t i = 0; i < driveCount; ++i)
	{
		f(drives[i]);
...
void Tool::IterateHeaters(function_ref_noexcept<void(int) noexcept> f) const noexcept
{
	for (size_t i = 0; i < heaterCount; ++i)
	{
		f(heaters[i]);
...
GCodeResult Tool::GetSetFeedForward(GCodeBuffer& gb, const StringRef& reply) THROWS(GCodeException)
{
	...
	uint32_t advance;
	if (gb.TryGetLimitedUIValue('A', advance, seen, 51))
	{
...
{
	IterateExtruders([](unsigned int n)->void { reprap.GetMove().ClearExtruderMovementPending(n); });
}
