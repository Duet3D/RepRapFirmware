...
bool Platform::SendUartData(size_t auxChannel, const uint8_t *data, size_t len) noexcept
{
	if (auxChannel >= NumSerialChannels - 1)
	{
		debugPrintf("auxChannel %u too high\n", auxChannel);
		return false;
	}
	...
	if (rslt != GCodeResult::ok)
	{
		debugPrintf("Failed to send data\n");
	}
