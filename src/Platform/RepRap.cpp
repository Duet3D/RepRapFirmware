...
constexpr ObjectModelTableEntry RepRap::objectModelTable[] =
{
	...
	{ "nextTool",				OBJECT_MODEL_FUNC((int32_t)self->gCodes->GetPrimaryMovementState().newToolNumber), ObjectModelEntryFlags::live },
#if HAS_VOLTAGE_MONITOR
	{ "powerFailScript",		OBJECT_MODEL_FUNC(self->gCodes->GetPowerFailScript()),					ObjectModelEntryFlags::none },
#endif
	{ "previousTool",			OBJECT_MODEL_FUNC((int32_t)self->gCodes->GetPrimaryMovementState().previousToolNumber),	ObjectModelEntryFlags::live },
...
OutputBuffer *RepRap::GetLegacyStatusResponse(uint8_t type, int seq) const noexcept
{
	...
	else if (type == 3)
	{
		...
		response->catf(",\"geometry\":\"%s\",\"axes\":%u,\"totalAxes\":%u,\"axisNames\":\"%s\",\"volumes\":%u,\"numTools\":%u,\"myName\":\"%.s\",\"firmwareName\":\"%.s\"",
						move->GetGeometryString(), numVisibleAxes, gCodes->GetTotalAxes(), gCodes->GetAxisLetters(), MassStorage::GetNumVolumes(), Tool::GetNumberOfContiguousTools(), myName.c_str(), FIRMWARE_NAME);
	}
