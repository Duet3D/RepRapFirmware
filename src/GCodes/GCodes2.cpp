...
std::string setArithmeticsExpressionValues(std::string& gExpression) {
	std::string resultExpression = "";
	unsigned int findingVariablesHeader = 0;
	bool again = false;
	for (auto& c : gExpression) {
		resultExpression += (char)toupper(c);
	}
	do {
		again = false;
		size_t found = resultExpression.find("#P", findingVariablesHeader);
		if (found != std::string::npos) {
			if (
				isdigit(resultExpression[found + 2]) &&
				((resultExpression.substr(found + 3, 1)) == "#")
				) {
				unsigned int slot = resultExpression[found + 2] - '0';
				resultExpression = resultExpression.replace(found, 4, std::to_string(arithmeticsMemory[slot]));
				findingVariablesHeader += (found + 3); // moving find start header forward to speed up the find process. This and the following if can be omitted
				if (findingVariablesHeader < resultExpression.length()) {
					again = true;
				}
			}
		}
...
std::string GCodes::setArithmeticsExpressionAxisPositions(std::string& gExpression) {
	// we know we are processing within a correct pair of curly brackets because caller is strictly checked
	std::string resultExpression = "";
	unsigned int findingVariablesHeader = 0;
	bool again = false;
	for (auto& c : gExpression) resultExpression += (char)toupper(c);
	for (int thisAxisNumber = 0; thisAxisNumber < 2; ++thisAxisNumber) {
		do {
			again = false;
			size_t found = gExpression.find(axisLetters[thisAxisNumber], findingVariablesHeader);
			if (found != std::string::npos) {
				resultExpression = resultExpression.replace(found, 1, std::to_string(currentUserPosition[thisAxisNumber]));
				findingVariablesHeader += found + 1; // moving find start header forward to speed up the find process. This and the following if can be omitted
				if (findingVariablesHeader < resultExpression.length()) {
					again = true;
				}
			}
