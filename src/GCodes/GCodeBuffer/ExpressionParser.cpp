...
namespace StackUsage
{
	...
	// except for GetObjectValueUsingTableNumber which is from ObjectModel.su.
	// The values for SAME70 MCUs used to be different from the values for other MCUs but currently they are identical.
#if SAME70
	constexpr uint32_t ParseInternal = 80;
	constexpr uint32_t ParseIdentifierExpression = 280;
	constexpr uint32_t ParseGeneralArray = 360;
	constexpr uint32_t ReadArrayFromFile = 392;
	constexpr uint32_t ApplyObjectModelArrayIndex = 112;
	constexpr uint32_t EvaluateTake = 128;
	constexpr uint32_t EvaluateDrop = 200;
...
void ExpressionParser::EvaluateTake(ExpressionValue& rslt, uint32_t arg, bool evaluate) const THROWS(GCodeException)
{
	switch (rslt.GetType())
	{
	case TypeCode::ObjectModelArray:
		{
			...
			ReadLocker locker(entry->lockPointer);
			const size_t len = min<size_t>(entry->GetNumElements(rslt.omVal, localContext), arg);
			ArrayHandle ah;
	...
	case TypeCode::HeapArray:
		{
			WriteLocker lock(Heap::heapLock);
			const size_t len = min<size_t>(rslt.ahVal.GetNumElements(), arg);
			ArrayHandle ah;
			ah.Allocate(len);
			for (size_t i = 0; i < len; ++i)
			{
	...
	case TypeCode::CString:
		{
			const size_t len = min<size_t>(strlen(rslt.sVal), arg);
	...
	case TypeCode::HeapString:
		{
			...
			const size_t len = min<size_t>(strlen(p.Ptr()), arg);
...
void ExpressionParser::EvaluateDrop(ExpressionValue& rslt, uint32_t arg, bool evaluate) const THROWS(GCodeException)
{
	switch (rslt.GetType())
	{
	case TypeCode::ObjectModelArray:
		{
			...
			const size_t offset = min<size_t>(numOriginalElements, arg);
	...
	case TypeCode::CString:
		{
			...
			const size_t offset = min<size_t>(slen, arg);
			...
			case Function::take:
				{
					...
					CheckStack(StackUsage::EvaluateTake);
					EvaluateTake(rslt, nextOperand.uVal, evaluate);
				}
				break;
			...
			case Function::drop:
				{
					...
					GetNextOperand(nextOperand, evaluate);
					ConvertToUnsigned(nextOperand, evaluate);
					CheckStack(StackUsage::EvaluateDrop);
					EvaluateDrop(rslt, nextOperand.uVal, evaluate);
				}
				break;
