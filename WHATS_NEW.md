...
New features and changed behaviour since 2.02RC2:
- Zero or negative Z probe tolerance (S parameter) in M558 with A parameter > 1 now means always average all readings
- When storage module debug is enabled, failing to open a file is now a warning not an error
- Increased number of restore points from 3 to 6
- G60 now saves the current tool as well as the current user coordinates
- Support T R# where # is a restore point number
- Support M600
- Temperature sensor type 300-307 now supports a C parameter to select the input channel and a D parameter to select differential mode
- When the WiFi module is in client mode it tries to auto-reconnect continuously if the connection is lost
- Implemented object model framework and a few variables, and M408 P1 command to extract values from the object model
- Support M208 Xaa:bb Ycc:dd etc. as an alternative to separate M208 S0 and M208 s1 commands
...
- M205 can be used to set the jerk limit as an alternative to M566
